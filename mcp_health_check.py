...
def check_mcp_server_health(url: str) -> bool:
    ...
    print(f"🔍 Checking Nikola TEST MCP MCP server health at {url}")
    ...
    if "error" in result and "session" not in str(result.get("error", "")):
        ...
        if "result" in result and "tools" in result["result"]:
            ...
            if missing_tools:
                ...
                print("✅ All expected Nikola TEST MCP tools are available!")
...
def main():
    ...
    print(f"🚀 Nikola TEST MCP MCP Server Health Check")
    ...
    if check_mcp_server_health(args.url):
        ...
        print("\n❌ Nikola TEST MCP MCP Server health check failed!")
        return 1
